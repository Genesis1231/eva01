...
def _get_llm() -> ChatPerplexity:
    ...
    if _llm_perplexity is None:
        _llm_perplexity = ChatPerplexity(temperature=0.1, max_tokens=120, timeout=30)
    return _llm_perplexity

@tool
async def web_search(query: str) -> str:
    """Search the web with a concise query. I use this when I need to look something up."""
    try:
