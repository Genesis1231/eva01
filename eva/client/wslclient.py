...
class WSLClient:
    ...
    async def receive(self, save_file: str=None) -> Dict:
        """ Receive the data from the client """
        
        observation = await asyncio.to_thread(self.watcher.glance)
        message, language = await asyncio.to_thread(self.listener.listen, save_file)
    ...
    async def speak(self, response: str, wait: bool= True) -> None:
        ...
        await asyncio.to_thread(self.speaker.speak, response, wait)
    ...
    async def deactivate(self) -> None:
        await asyncio.to_thread(self.watcher.deactivate)
