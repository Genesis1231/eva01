    ...
    async def _process_queue(self) -> None:
        ...
        while True:
            try:
                ...
                match data_type:
                    case "audio":
                        audio_data = await asyncio.to_thread(convert_audio_data, content)
                        result = await asyncio.to_thread(self.transcriber.transcribe, audio_data)
                    
                    case "frontImage" | "backImage":
                        result = await asyncio.to_thread(self.img_describer.describe, "vision", content)
